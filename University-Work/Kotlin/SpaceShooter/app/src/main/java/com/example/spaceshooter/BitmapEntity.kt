...
open class BitmapEntity : Entity() {
    ...
    open val collisionBoxScale = 0.5f

    fun setSprite(bmp: Bitmap) {
        bitmap = bmp
        width = bitmap.width.toFloat() * collisionBoxScale
        height = bitmap.height.toFloat() * collisionBoxScale
    }
    override fun render(canvas: Canvas, paint: Paint) {
        canvas.drawBitmap(bitmap, x, y, paint)

        // Debug: Draw the bounding box in red
